...
{
    var user = this.model.at(el).get();
    this.model.set('user.' + user.id + '.kvargCount', parseInt(user.kvargCount) + 1);
});

app.fn('user.dec', function(e, el)
{
    var user = this.model.at(el).get();
    var count = parseInt(user.kvargCount);
    this.model.set('user.' + user.id + '.kvargCount', count > -2 ? count - 1 : count);
});
